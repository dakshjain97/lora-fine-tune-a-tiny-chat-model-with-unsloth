...
def load_base_model_and_tokenizer(model_name='unsloth/Qwen2.5-0.5B-Instruct-bnb-4bit', max_seq_length=256):
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(model_name, max_seq_length,  load_in_4bit=True)
...
def is_model_4bit_quantized(model):
    ...
    linear_4bit_cls = getattr(getattr(bnb, "nn", None), "Linear4bit", None)
     # Walk the submodules and check types
    for module in model.modules():
        if isinstance(module, linear_4bit_cls):
            return True
    return False
